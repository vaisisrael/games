/* wordstack.js – Parasha "תיבה ואות" game (module)
   FLOW UPDATED:
   ✔ המילה של המחשב לא ננעלת עד לחיצה על "תורי ▶"
*/
...
(() => {
  ...
  function render(rootEl,model){
    ...
    async function computerTurn_(){
      ...
      // ❗ חשוב — לא נועלים עדיין
      state.pendingComputerWord = next;

      // מציגים רק בתיבה העליונה
      elInput.value = next;
    ...
    btnMain.addEventListener("click", async ()=>{
      ...
      if(state.turn==="afterComputer"){
        ...
        // ✔ עכשיו בלבד — נועלים את מילת המחשב
        if(state.pendingComputerWord){
          state.word = state.pendingComputerWord;
  ...
  (function registerWhenReady(){
    ...
    setTimeout(registerWhenReady,30);
  })();
